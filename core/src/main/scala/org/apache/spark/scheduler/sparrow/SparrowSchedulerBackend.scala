...
/**
  * A scheduler backend that waits for sparrow executors to connect.
  */
private[spark]
class SparrowSchedulerBackend(scheduler: SparrowScheduler)
  ...
  class DriverEndpoint(override val rpcEnv: RpcEnv, sparkProperties: Seq[(String, String)])
    ...
    override protected def log = SparrowSchedulerBackend.this.log
    ...
    override def receiveAndReply(context: RpcCallContext): PartialFunction[Any, Unit] = {
      ...
      case RegisterExecutor(executorId, executorRef, cores, logUrls) =>
        ...
        if (executorDataMap.contains(executorId)) {
          ...
          SparrowSchedulerBackend.this.synchronized {
    ...
    def removeExecutor(executorId: String, reason: ExecutorLossReason): Unit = {
      executorDataMap.get(executorId) match {
        case Some(executorInfo) =>
          // This must be synchronized because variables mutated
          // in this block are read when requesting executors
          val killed = SparrowSchedulerBackend.this.synchronized {
  ...
  protected def createDriverEndpointRef(
    ...
    rpcEnv.setupEndpoint(SparrowSchedulerBackend.ENDPOINT_NAME, createDriverEndpoint(properties))
  }
...
private[spark] object SparrowSchedulerBackend {
  val ENDPOINT_NAME = "SparrowSchedulerBackend"
}
