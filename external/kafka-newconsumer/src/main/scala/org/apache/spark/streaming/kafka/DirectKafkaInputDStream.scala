...
class DirectKafkaInputDStream[
  ...
  @transient protected var currentOffsets: Map[TopicPartition, Long] = null


  protected final def latestOffsets(): Map[TopicPartition, LeaderOffset] =
    kc.getLatestOffsetsWithLeaders(currentOffsets.keySet)



  // limits the maximum number of messages per partition
  protected def clamp(
    leaderOffsets: Map[TopicPartition, LeaderOffset]): Map[TopicPartition, LeaderOffset] = {
  ...
  override def compute(validTime: Time): Option[KafkaRDD[K, V, R]] = {
    currentOffsets = serCurrentOffsets.map { i =>
      new TopicPartition(i._1, i._2) -> i._3
    }.toMap
