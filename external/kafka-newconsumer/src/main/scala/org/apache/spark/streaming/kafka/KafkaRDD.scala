...
class KafkaRDD[
  ...
  private class KafkaRDDIterator(
    ...
    val consumer = new KafkaConsumer[K, V](kafkaParams.asInstanceOf[Map[String, Object]].asJava)
    consumer.assign(List(new TopicPartition(part.topic, part.partition)).asJava)
    ...
    override def getNext(): R = {
      ...
      if (!iter.hasNext) {
        ...
        if (item.offset >= part.untilOffset) {
          ...
          requestOffset = item.offset + 1; // mario hardcoded in original kafka as well
          messageHandler(item)
        }
      }
    }
